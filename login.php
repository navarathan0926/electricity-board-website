...
if(isset($_POST['submit'])){
   ...
   $select_users = mysqli_query($conn, "SELECT * FROM `users` WHERE email = '$email' AND password = '$psw'") or die('query failed');
   $select_admin = mysqli_query($conn, "SELECT * FROM `admins` WHERE email = '$email' AND password = '$psw'") or die('query failed');
   if(mysqli_num_rows($select_users) > 0){
      $row = mysqli_fetch_assoc($select_users);
         $_SESSION['acc_no'] = $row['acc_num'];
        ...
        $rows = mysqli_fetch_assoc($select_admin);
